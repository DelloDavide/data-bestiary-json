...
7. **Release ZIP non versionata semanticamente**: il tag è `<branch>.<timestamp>`. Nessun changelog, nessun `latest` stabile.
8. **Caratteri speciali nei testi degli effetti**: presenza mista di apostrofi tipografici (`'`) vs ASCII (`'`) e di emoji come `icon` → potenziali problemi di render/diff.
9. **`ev` / `stamina` come stringa**: il plugin parsa come stringa per accomodare unità (`"12 per square"`, `"4 per 10 x 10 patch"`). Cambiare a numero romperebbe il dynamic terrain.
